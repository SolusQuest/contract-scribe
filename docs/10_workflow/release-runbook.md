...
claims only that these exact bytes were produced by the trusted
...
always carries provenance. Whether consumers *verify* the attestation is
a separate R3-gated decision (#189); adding acquisition-path enforcement
later changes the wrapper revision and would require its own issue and
re-qualification.
