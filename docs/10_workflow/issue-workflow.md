...
Before declaring pre-release work agent-ready, apply [Pre-release engineering proportionality](../00_project/pre-release-engineering.md). Its process-complexity checkpoint is mandatory when one primary outcome plans multiple ordered pull requests or human merges, post-merge mutations, repeated independent review or full validation, compatibility machinery, manual ceremony that duplicates machine state, or reopening closed historical work. Record the complete process profile in the issue. If an extra stage does not protect a distinct current failure and lacks evidence against a simpler reversible alternative, remove or combine it.
