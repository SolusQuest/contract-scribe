...
M0 is complete, and M1 has implemented the production in-process audit path: explicit Roslyn/MSBuild loading, ADR 0003 target classification and direct documentation observation, policy evaluation, bounded evidence, canonical result production, and the atomic `ProductionAuditHost`. Issue #75 removed the retired validation and experiment machinery. Remaining M1 work is #41 exact-main validation, #30 CLI implementation, and #42 independent read-only smoke. Proposal generation, safe source modification, campaign state, GitHub write operations, and a consumable GitHub Action belong to later milestones and are not yet implemented.
