...
3. **Release freeze or certification gate** — the first downstream-consumable release decides whether its concrete external-consumer or irreversible-publication boundary requires fixed release inputs, a persistent release-authorizing content identity, independent acceptance, signing, or a signed release attestation.
...
Before the first downstream-consumable release, Host Validation and other workflow-based repository validation use the applicable exact source commit, workflow revision, run and attempt, runner and toolchain observations, and run-generated manifest or artifact digests as provenance. Those digests may make one run internally consistent, but they are outputs of that run rather than a checked-in authorization object. They do not create a Host protected-input set that later pull requests must refresh, an accepted snapshot that later development must preserve, or a repository review record that authorizes execution. Other capabilities use the minimum provenance defined by their owning contract.

This rule does not prohibit exact-revision milestone evidence, bounded public or private smoke attestations, draft contract-baseline or provenance identities, or snapshot, work-plan, batch, campaign, ledger, cursor, checkpoint, and operation identities required for deterministic replay, stale-state rejection, or idempotent mutation during M1 through M5. Those identities protect their named pre-release contract boundary; they are not placeholder release authorization.
