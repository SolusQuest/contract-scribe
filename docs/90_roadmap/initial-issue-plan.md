...
| M0.2 | Define Audit Result v1, canonical JSON, diagnostics, and public fixtures. | M0.1 |
| M0.3 | Define symbol identity, target taxonomy, evidence taxonomy, generated-code policy, locators, and fixture strategy. | None |
| M0.4 | Prove framework-dependent Roslyn/MSBuild loading and deterministic semantic payload on the primary synthetic fixture. | M0.1, M0.2, M0.3 |
| M0.5 | Attempt Native AOT on the exact M0.4 semantic path and record supported or bounded not-feasible evidence. | M0.4 |
| M0.6 | Compare M0.4 and M0.5 evidence and record the selected or no-selection loader/distribution baseline in an ADR. | M0.5 |
