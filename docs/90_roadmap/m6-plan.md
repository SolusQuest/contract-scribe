...
`#183` (C1, this issue) establishes the public consumer configuration layer: payload defaults, an optional downstream file, and an optional invocation override merged inside the production C# CLI with declared-field, case-sensitive, closed-shape semantics. Invocation authority (campaign lineage, snapshot binding, state path) and product-derived identity are CLI inputs or derived facts, never consumer-layer content. The resolved output is the complete `campaign-configuration-v1` runtime-authority document; `github-proposal --configuration` keeps consuming that authority document directly until `#184` (C2) replaces its invocation surface with the resolved-settings contract and explicit runtime authority.
