...
- Required CI may execute that same coverage as independent runner jobs. The current partition is the complete fast suite plus an exact campaign crash-boundary integration case and its exact complement; the union must preserve the unfiltered integration full-name multiset.
...
CI runner parallelism is separate from xUnit worker parallelism. The campaign crash-boundary case and the exact complementary integration set run on different hosted runners, so they cannot share filesystems, process trees, build servers, environment mutation, or process inventories. Do not add another CI partition until its complete producer/consumer and shared-state boundaries are established and an unfiltered-versus-partitioned TRX comparison proves identical discovered and executed full-name multisets. Do not raise the assembly worker count as a CI-speed shortcut.

The equality filter currently names `ContractScribe.Roslyn.IntegrationTests.CampaignCliProcessTests.EveryFrozenBoundary_IsAcknowledgedTerminatedAndReinvokedAgainstTheSameCheckpoint`; the complementary job uses the same full name with `!=`. Both jobs fail when their filter discovers zero tests. A rename therefore fails closed instead of silently dropping coverage. After a filter or discovery change, compare the partition TRX with an applicable unfiltered execution, including duplicate Theory identities. Reuse an existing unfiltered run when production, tests/data, fixtures, projects/packages, and build inputs are unchanged; a workflow or prose-only commit does not by itself require another full execution. Otherwise use the `workflow_dispatch` input `include_unfiltered_evidence=true` to obtain the missing unfiltered evidence.
...
Exact-head GitHub-hosted Ubuntu x64 CI is the final required platform leg, but it does not replace local focused validation. In CI the complete fast suite, formatting, and all CLI smokes share one runner; a continued fast-suite failure does not suppress the static/smoke observations. The two integration partitions run on separate runners with separate TRX artifacts. Each worker aggregates its original outcomes, and the stable `validate` job succeeds only when all three required workers succeed. The campaign crash partition retains a 35-minute hard timeout; the remaining partition has 45 minutes, and optional unfiltered evidence has 55 minutes because it includes both workloads. No automatic retry is added. The H2 fresh-process response-loss matrices and regressions expanded the remaining workload: its first corrected-head hosted run reached the former 35-minute cap without TRX, while a subsequent complete WSL diagnosis passed all 773 cases in 35 minutes 28 seconds. That development observation explains the finite budget margin; it does not replace exact-head hosted CI or establish a performance guarantee. The remaining and unfiltered commands print per-test progress so an interrupted run retains useful diagnostics. These CI hard caps are distinct from the local outer observation budgets above, which require inspecting and preserving an active original run rather than killing and restarting it. Artifact names include `github.run_attempt`, so an explicitly initiated rerun preserves its own TRX without colliding with an earlier attempt.
