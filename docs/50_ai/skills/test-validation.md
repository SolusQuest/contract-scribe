...
Exact-head GitHub-hosted Ubuntu x64 CI is the final required platform leg, but it does not replace local focused validation. In CI the fast and integration suites remain separately named, write separate TRX artifacts, both run after a successful build even if one fails, and an ordinary final step aggregates their original outcomes. The integration step has a 35-minute hard timeout and no automatic retry. This is distinct from the 25-minute local outer observation budget above: the CI cap preserves teardown and TRX-finalization margin after the hosted run approaches that observation threshold. Artifact names include `github.run_attempt`, so an explicitly initiated rerun preserves its own TRX without colliding with an earlier attempt.
