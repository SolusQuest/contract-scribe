...
The thirty-iteration Windows causal-topology guard remains in process lane 2 and still executes thirty real two-process iterations. It uses exactly two isolated fixture pairs, each running fifteen iterations sequentially, so one slow serial guard does not dominate the full-suite wall clock. Do not share writable roots between the two streams or raise this internal concurrency without repeating the full timing and process-cleanup evidence.
