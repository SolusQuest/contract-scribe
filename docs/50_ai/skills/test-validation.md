...
Fixture preparation disables MSBuild node reuse for `dotnet restore`, `dotnet build`, and direct `dotnet msbuild` commands; compilation commands also disable the shared compiler process. Keep that ownership boundary when adding preparation commands: a reusable node can outlive the command root, retain redirected output handles, and make the test runner wait on a process it no longer owns through the original root. Do not replace this narrow process-lifetime policy with `--disable-build-servers`: hosted evidence showed that the broader switch discarded useful build caches and more than doubled integration latency.
