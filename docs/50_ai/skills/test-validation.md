...
The equality filter currently names `ContractScribe.Roslyn.IntegrationTests.CampaignCliProcessTests.EveryFrozenBoundary_IsAcknowledgedTerminatedAndReinvokedAgainstTheSameCheckpoint`; the complementary job uses the same full name with `!=`. Both jobs fail when their filter discovers zero tests. A rename therefore fails closed instead of silently dropping coverage. After any filter or discovery change, use the `workflow_dispatch` input `include_unfiltered_evidence=true`, download all three integration TRX artifacts, and compare sorted full-name multisets including duplicate Theory rows before accepting the new partition.
...
Pull-request CI uses one concurrency group per PR and cancels obsolete heads. Manual evidence and push runs use the unique workflow run ID and are never cancelled by a sibling sample. Logs record the computed group and cancellation policy with the runner/toolchain facts. Performance qualification uses first-attempt, non-overlapping manual runs at one exact commit; do not use reruns, cancelled runs, or mixed runner/toolchain environments as samples.
