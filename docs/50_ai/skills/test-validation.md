...
Exact-head GitHub-hosted Ubuntu x64 CI is the final required platform leg, but it does not replace local focused validation. In CI the complete fast suite, formatting, and all CLI smokes share one runner; a continued fast-suite failure does not suppress the static/smoke observations. The two integration partitions run on separate runners with separate TRX artifacts. Each worker aggregates its original outcomes, and the stable `validate` job succeeds only when all three required workers succeed. Integration partitions retain a 35-minute hard timeout and no automatic retry. This is distinct from the 25-minute local outer observation budget above: the CI cap preserves teardown and TRX-finalization margin after the hosted run approaches that observation threshold. Artifact names include `github.run_attempt`, so an explicitly initiated rerun preserves its own TRX without colliding with an earlier attempt.
