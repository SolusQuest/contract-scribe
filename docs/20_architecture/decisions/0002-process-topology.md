...
Analyzed repositories are trusted input (R1). The host provides no sandbox and claims none. Environment handling follows three bounded guarantees: ContractScribe does not require or intentionally consume provider, model, or GitHub credentials; credential-like values must never be persisted or emitted through diagnostics, results, logs, or evidence; and repository-controlled MSBuild evaluation runs with the OS-inherited environment under the trusted-input assumption and is not sandboxed. MSBuild evaluation itself reads environment variables as project properties, so a stronger never-read guarantee would require an explicit scrubbed-environment design and is a decision reconsideration trigger (§12).

Relationship to the security boundary: the `security-boundary.md` requirement that the deterministic audit must not access a network, provider secret, or GitHub write token is honored here as a ContractScribe behavior and dependency boundary — ContractScribe itself never requires, consumes, or emits them, and the evidenced M0.4/M0.7 path runs without requiring them. It is not a confidentiality boundary against repository-controlled build logic: under R1, MSBuild targets, analyzers, and generators execute with the caller's privileges, and in-process environment filtering cannot make an untrusted repository safe — that would require a sandbox, which no candidate claims. Callers handling repositories of reduced trust with credentials present should run the audit in a credential-free environment. A mandatory scrubbed or allowlisted environment is not part of this decision, because in-process filtering cannot constitute a real security boundary; if a future requirement demands one, it triggers decision reconsideration (§12). The audit never modifies source or project files itself; build outputs exist only because the caller prepared them, and any repository writes caused by MSBuild/design-time evaluation (for example `obj` artifacts) are bounded and verified by #24 and #26 rather than claimed absent here. Non-publication temporary files go to the system temporary directory with audit-unique names and deletion on exit; result-publication staging follows §9 instead. Paths are constrained lexically to the repository root; symlink, junction, or reparse-point escapes outside the root are rejected as invalid input.
...
| Reconciliation with `security-boundary.md`: behavior/dependency boundary, not a confidentiality boundary against trusted build logic | decided | this section |
...
- Execution outcome: invalid input, environment unavailable, load failure, audit error, publication failure, cancelled, timeout.
...
| Publication-failure windows: failed run-start invalidation is not-selected; failed final atomic replacement is selected; both commit one non-success terminal, expose no current result, clean any staging, and reject late success | decided | this section; exact vectors and direct artifact observations in [#26](https://github.com/SolusQuest/contract-scribe/issues/26) |
