...
> **Status:** Current pre-release M4 local campaign boundary and M5-R1–R6 GitHub adapter. `CampaignPlanner`, Campaign State v1, the C3 reducer, X2A checkpoint store, X1B/X1C execution, and the production campaign CLI implement snapshot-scoped planning, durable same-snapshot recovery, and caller-attested changed-base supersession. The internal `GitHubPublicationReconciler` now composes authenticated coordination, exact Git data/ref publication, and PR ownership through the existing Core port. GitHub proposal CLI composition and live host proof remain H2/H3 work; automatic rebase, ready, merge, close, deletion, and Action scheduling remain deferred.

R6 performs one proposal transition or one PR create per port call. Ordinary first publication uses two calls on the same authority-bound adapter: exact proposal publication followed by a single draft create. Private acknowledged-winner receipts authorize the next write; replay or restart never recreates those receipts. Lost authority can therefore leave a verified partial operation that only supports read-only recovery. Complete resource checks cover cumulative append, original immutable PR metadata, publication holds, and fresh terminal successors. An initial stale-create failure is recorded as incomplete `stale-draft`; subsequent reconciliation is read-only while that draft remains active. Trusted host principal composition and actual token-kind/permission proof retain the boundaries in [Security boundary](security-boundary.md).
