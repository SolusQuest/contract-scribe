...
1. A cross-cell source/configuration manifest binds the exact Issue #24 source revision and a closed, generated inventory from the frozen roots `Directory.Build.props`, `Directory.Packages.props`, `global.json`, and `src`. It also binds the named failure registry, calibrated-bound manifest, build recipe, command/control contract, contract baseline, environment policy, and validation workflow paths frozen in `protocol.json`. The harness resolves that inventory from the host revision's Git tree, verifies that the host revision is an ancestor of the validation-execution revision, and hashes every Git blob before a production run. Current-checkout materialization must match those commit-bound identities. Every named recipe or policy is a path-plus-content identity, and `sourceConfigurationId` is the canonical digest of the complete ordered record. `execution-subject.template.json` is deliberately non-executable: Issue #24 or the execution issue must expand every frozen root to the exact ordered Git-tree inventory and replace all template identities.
...
Audit and execution fields are absent when no managed terminal record can exist. When a canonical Audit Result exists, the harness independently validates it against the pinned v1 schema and derives the Audit Result version, policy/configuration version, taxonomy-registry version, target profile, ordered audit outcomes, and canonical commitment from its bytes. Vector observations and equality checks use those independently derived facts rather than trusting the subject response. Every handled failure binds the exact protected Issue #24 failure-registry identity, observed production failure code, normalized failing stage, terminal-commit state, and artifact state. This protocol does not invent Issue #24 codes.
...
The harness records the root subject synchronously before polling descendants, then distinguishes additional ContractScribe workers, toolchain-owned processes, and unknown descendants. Windows samples use one Toolhelp process snapshot; Unix samples tolerate processes that disappear between enumeration and inspection but mark any other inspection failure incomplete. Claims about zero workers, no restore initiation, or other short-lived descendants require the protected synchronized-tree arrangement and a successful post-gate sample; bounded polling alone yields incomplete evidence. A child `dotnet` process is fail-closed as another ContractScribe worker unless the protected arrangement proves it is toolchain-owned. Missing-assets/no-restore rejects an exact restore-output marker or any observed descendant during the synchronized interval.
