...
The no-ContractScribe-initiated-network-operation row combines the synchronized descendant observation, a harness-selected empty operation-recorder path implemented by the Issue #24 test seam, and independent scans over the exact protected production-source inventory and complete local transitive managed dependency closure. The source scan strips comments and literals before rejecting direct `System.Net` namespaces, network client/socket/DNS types, and connection/send factories; the managed-metadata scan follows local assembly references and rejects compiled member references whose declaring namespace is `System.Net`. Every local assembly in that closure must be listed and hash-bound by the cell materialization. A missing or nonempty recorder, omitted helper dependency, forbidden source or compiled seam, or protected restore/runtime-download/unknown descendant makes the vector nonconforming. This is a trusted-source behavior check, not an egress-control claim.

The forced-termination row uses the production subject entry point, waits for the exact `forced-termination` gate, performs the harness-owned `external-kill`, and derives its observation only from an `issued-and-observed` process-tree kill plus absence of a valid result. Already-exited, permission-failure, unsupported, indeterminate, or normal-exit races cannot be relabeled as external termination.

Cancellation and publication transition rows receive a harness-selected bounded transition-recorder path. Ordered events prove invalidation before the first failure-prone stage, same-destination staging before atomic rename, the first committed terminal outcome, and rejection of competing or late terminal attempts. Final state and the response observation code alone cannot satisfy those rows. Once a handled non-success or canonical result is committed, that terminal outcome remains authoritative even if the process later crashes, aborts, or is externally killed.
...
Public validation evidence is canonical JSON encoded as UTF-8 without BOM, compact, with object keys in ordinal order and exactly one trailing LF. Audit Result v1 is different: the harness parses strict UTF-8 and duplicate-free JSON without applying the generic alphabetical canonicalizer, validates schema and semantics, reserializes through the protected Audit Result v1 canonicalizer with its normative property and typed array orders, and compares those exact bytes. The per-cell limit is 4 MiB and subject-response limit is 256 KiB.
