...
Issue #26 remains the closed historical origin of the protocol, schemas, vectors, harness, self-tests, and independent review record. Issue #70 owns a one-time successor correction to baseline identity, lineage validation, the contract-to-Host consumer seam, and deterministic pending-review maintenance; it does not reopen or rewrite Issue #26. Neither issue implements the production host, executes the final matrix against a claimed host revision, or publishes a passing host aggregate. Issue #24 owns the real production composition and the prebuilt test-only production subject entry point. Issue #57 later reconciles and independently accepts the exact bundle. After Issue #57 closes, Issue #41 selects and materializes the production subject entry point and executes the accepted bundle unchanged.
...
The contract baseline owns `M1ContractBaselineHostConsumerTests.cs` as the dedicated full-document consumer of the classification origin/skip matrix through Host semantic validation. The broader `M1HostValidationProtocolTests.cs` remains Host lifecycle and certification infrastructure. Changing the lifecycle suite alone is ordinary Host protected-input drift; changing the dedicated consumer or another current manifest input requires a successor contract baseline.

## Frozen identities

The protocol binds the Issue #70 successor baseline and its complete Issue #55 predecessor identity. `baseline.coordinatingIssue` and `baseline.contractRevision` identify the current Issue #70 revision, while `baseline.predecessor` pins Issue #55 revision `issue-55-classification-origin-closure-v1`, exact commit `95933c5dc134dfe6adeb92765920a8eb5c96d7db` (`S1`), the manifest path, and digest `e89c1769ca7f725bd813d345023bfcbcf57319ffc11268423d57b6b304999a85`. The validator always proves that exact predecessor commit exists, is an ancestor of the validating checkout, and contains the pinned historical manifest bytes, including while the current baseline is pending. Every commit-existence, ancestry, tree, and blob query uses the raw Git object graph with replacement refs disabled, and a non-empty repository graft file is rejected before lineage can be claimed. In the Issue #70 candidate, `baseline.disposition` is `pending-main-reconciliation`, `mergeCommit` is JSON `null`, and the current review record is non-authorizing. A later reconciliation may change the disposition to `main-reachable` only after the exact Issue #70 successor manifest bytes exist in a 40-character lowercase commit that descends from `S1` and is an ancestor of the validating checkout. The validator reads those exact manifest bytes from that commit before any production evidence operation can proceed. The protected-input manifest records the byte identities of the baseline contracts, schemas, fixtures, conformance code, ADR 0002, the accepted Audit CLI contract, the security boundary, and project-structure rules. Issue #24 remains a mutable live ownership trace and is never represented as a hash-bound source document.

Issue #70 closes only after the human-created `C2` squash commit is reachable from `main`, the structurally valid candidate remains `pending-main-reconciliation` and non-authorizing, and the required repository and tracker readback completes. It does not own the later accepted bundle. Issue #57 waits until the production Host and every active protected-input producer required by the execution gate are stable, then owns the `main-reachable` reconciliation and independent accepted review before exact-revision production evidence execution.
...
- `S1`, or contract baseline `C1`, is the immutable Issue #55 squash commit and predecessor identity;
- `C2` is the Issue #70 squash commit and owns the exact Issue #70 `contractRevision`, manifest path, and manifest digest;
- `Bc` is any structurally current pending candidate identity in the lineage after `C2`;
- `P0'` is the later stable main revision from which Issue #57 certification begins and is recorded as the promotion source, not substituted for the contract baseline;
...
The pending command matrix is closed. `lock-protected-inputs`, `lock-bundle`, `lock-pending-review`, `validate-bundle` without `--require-review`, `dry-run`, and `self-test` are structural or maintainer operations and remain available. `lock-pending-review` derives the canonical pending record and deterministic review ID from the already locked current bundle; repeated invocation over unchanged inputs produces identical bytes. `validate-bundle --require-review`, `provision-fixtures`, `run-cell`, `validate-cell`, `validate-incomplete`, `aggregate`, `validate-aggregate`, `validate-publication-record`, and `prepare-public` reject with `HV246_BASELINE_NOT_MAIN_REACHABLE` before consuming or writing evidence while the baseline is pending. After main reconciliation, the same commands reject a still-pending checked-in review with `HV121_REVIEW_NOT_ACCEPTED` until an exact accepted review is supplied.
...
`lock-protected-inputs`, `lock-bundle`, and `lock-pending-review` are maintainer commands and run in that order after all protected code, tests, schemas, and documentation are final. Any later protected or direct bundle change restarts the applicable sequence. Any regenerated identity requires a new exact review. Once the baseline is main-reachable and the exact bundle has an accepted review, `provision-fixtures` writes only the frozen deterministic runtime fixture inventory and rejects any remaining extra or changed file through the recipe identity. The CI `validate-bundle`, `dry-run`, and `self-test` executions validate the oracle and harness on both operating systems; they are not production-host pass evidence.
