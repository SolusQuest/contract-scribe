...
`executorKind` is executable, not documentary. `production-host` invokes only the Issue #24 entry point. `external-process` invokes the protected per-vector executable and argument arrangement. `platform-fixture` invokes an already provisioned platform arrangement whose recipe and artifacts are protected inputs. `harness-static` invokes a closed code registry; it cannot manufacture a pass from the vector's expected value. Helpers, arguments, provisioning inputs, expected external fatal cause, result location, design-time allowlist, synchronized observation mode, and bounded process-identity registry are bound by each cell's fixture realization. Available external and platform fixtures use only the exact `missing-executable` sentinel, the system-allowlisted `dotnet` launcher, or a `repository:` executable. Every fixed argument is either one of the four closed request/response/repository/control placeholders or a `repository:` reference present in the arrangement or built-artifact inventory; arbitrary non-path literals are rejected. A repository executable additionally carries its own SHA-256. The canonical execution-subject manifest hash binds this complete command and arrangement identity.
