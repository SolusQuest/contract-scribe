...
The forced-termination row uses the production subject entry point, waits for the exact `forced-termination` gate, and performs the harness-owned `external-kill`. On Windows the harness terminates descendants first, calls `TerminateProcess` on the exact root handle with frozen sentinel `0xE02600F1`, waits, and requires `GetExitCodeProcess` to equal that sentinel. On Linux the external-kill branch exclusively consumes root-child status through raw `waitpid`; before that capture it never calls `HasExited`, `WaitForExit*`, `ExitCode`, exit events, or another managed status consumer. Gate liveness uses a non-reaping native probe. It kills descendants first and the root last with `SIGKILL`, then requires `WIFSIGNALED` and `WTERMSIG == 9`. The normal-exit race regression is a deliberately non-causal control: it registers managed exit observation before a synchronized release, issues no kill request, and requires normal `exit(137)` to remain unsupported as native cause instead of relabeling the numeric status as `SIGKILL`. It never enters the post-gate sample or kill branch and uses no elapsed delay. Only an exact native cause plus a harness-issued request and no valid result derives `issued-and-observed`. Already-exited, permission-failure, unsupported, indeterminate, ambiguous, or normal-exit races cannot be relabeled as external termination.
