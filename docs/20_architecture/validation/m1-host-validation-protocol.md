...
The `toolchain.missing-assets` and `toolchain.no-automatic-restore` vectors intentionally share the exact `toolchain.assets-missing` fixture while assigning their claims to different evidence owners. The production subject reports the actual missing-assets classification and its `internally-enforceable` enforcement class for both invocations; it must not branch on either vector identity or expected oracle fields. For `toolchain.no-automatic-restore`, the harness disregards that subject-owned observation/enforcement pair and derives `toolchain.no-restore-observed` with `observable-only` enforcement only when its independent synchronized process-tree observation contains no `restore-or-runtime-download` role and its repository delta contains no created or changed `project.assets.json`. Either marker instead derives `toolchain.restore-or-runtime-download-marker-observed`. This claim-specific oracle derivation does not translate candidate output and does not alter the subject-owned missing-assets claim.
