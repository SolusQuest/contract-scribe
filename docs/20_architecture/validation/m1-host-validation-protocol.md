...
The no-ContractScribe-initiated-network-operation row combines the synchronized descendant observation, a harness-created empty operation-recorder file implemented by the Issue #24 test seam, and independent scans over the exact protected production-source inventory and complete local transitive managed dependency closure. The source scan strips comments and literals before rejecting direct `System.Net` namespaces, network client/socket/DNS types, connection/send factories, and network-capable indirection through reflection, dynamic dispatch, assembly loading, native libraries, P/Invoke, or unmanaged function pointers. The managed-metadata scan follows local assembly references and rejects compiled `System.Net` member references, the corresponding reflective/dynamic/native entry points, and P/Invoke method definitions. Every local assembly in that closure must be listed and hash-bound by the cell materialization. A missing or nonempty recorder, omitted or dynamically loaded helper dependency, forbidden source or compiled seam, or protected restore/runtime-download/unknown descendant makes the vector nonconforming. This is a trusted-source behavior check, not an egress-control claim.

The forced-termination row uses the production subject entry point, waits for the exact `forced-termination` gate, performs the harness-owned `external-kill`, and records the kill-request result separately from the final platform termination status. It derives `issued-and-observed` only after process exit reports a platform-compatible forced-termination status and no valid result exists. Already-exited, permission-failure, unsupported, indeterminate, zero/normal, ambiguous, or post-liveness-check normal-exit races cannot be relabeled as external termination.
