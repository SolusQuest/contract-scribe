...
If a scenario returns an unexpected CLI result, the driver emits only a bounded projection of known public terminal/outcome/operation/diagnostic codes, a bounded exit code and expected-baseline equality, then stops. Unknown values, raw stdout/stderr, provider data, paths and source are never reproduced. A failed scenario produces no success marker or transferable checkpoint and grants no retry. These diagnostics distinguish an observed terminal result; they do not establish an unreported internal or platform cause.
...
Before source review, run the applicable affected validation under the repository test procedure. Reuse unchanged product build/test evidence only when its source, tests, helper, SDK/package and build inputs remain valid; run the complete changed H3 suite. Check the offline job explicitly on the exact pushed head alongside ordinary CI/CodeQL; ordinary .NET CI alone does not execute this Python suite. Source review precedes separately authorized inert installation and finite Stage B. Final Ready follows complete live proof and final feedback/evidence review, with both source and final evidence reviews counted within the same bounded PR convergence budget. A failed run stops, consumes its authority and keeps the proof incomplete; do not retry or expand permissions to obtain a green result.
