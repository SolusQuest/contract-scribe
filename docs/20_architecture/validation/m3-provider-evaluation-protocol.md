...
Every repository tool definition binds `scopeId` to the sorted closed enum of request-visible opaque scope IDs authorized for that exact operation. A model never has to infer whether a source, maintained-context, or instruction scope permits read, list, or search. The terminal prompt separately names the exact proposal and skip root-property sets, identifies `componentIdentity` as the content-unit field, and marks `applicableComponents` and `identity` as guidance-only rather than terminal properties. These provider-facing constraints do not relax the production decoders or Core terminal validation; output outside the closed tool or terminal contract still fails closed.
