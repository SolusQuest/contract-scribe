...
Status: Commit-pinned pre-release draft; implementation (#30) and end-to-end validation (#26) pending.
...
This contract and its annex are a commit-pinned draft under [Contract lifecycle](../00_project/contract-lifecycle.md). The annex records `decisionInputCommit: 0806a98609b0eb97014496dcc4f4c8083ec57533` only as the historical repository revision at which the CLI decision input was authored; it does not claim that the current protected bytes existed at that commit. `protectedInputDisposition: current-coordinated-candidate` identifies the SHA-256 map as the exact current pre-merge Policy, Taxonomy, Audit, fixture, and conformance candidate consumed by this CLI contract. The checker fails on any single-file drift (partial-baseline rejection). Issue #55 owns the successor candidate and the exact S1 main-reconciliation gate; neither this historical decision-input commit nor PR1 predicts S1.

The protected conformance closure includes both the shared test-only Audit Result oracle `tests/ContractScribe.Tests/AuditResultConformance.cs` and the canonical byte implementation it delegates to, `tests/ContractScribe.ContractBaselineProbe/AuditResultCanonicalizer.cs`. The pins and canonical fixtures are regenerated when the accepted #35 baseline merges or whenever either protected source changes. Run classification itself depends only on `auditOutcome` values, so a #35 contract amendment that preserves the outcome vocabulary does not change classification semantics.
...
3. Audit Result `reasonCode` values, owned by the M0.2/#35 Audit Result contract. The CLI never redefines them, never uses them for classification, and carries them only as verbatim data inside the `cli.audit.skipped-summary` record.
