...
The GitHub adapter reconciles campaign state with a GitHub Issue, branch, commit, and pull request. It is the only product component that needs a GitHub write token.
...
| GitHub adapter | GitHub only | Yes | Proposal branch/worktree only | Publication record |
| Action wrapper | Payload acquisition only | Pass-through only | No direct product write | Action outputs and summary |
