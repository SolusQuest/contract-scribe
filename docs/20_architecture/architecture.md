...
The current candidate M5 graph separates `Core`, `Roslyn`, `Patching`, `Agent`, `GitHub`, and `Cli`. `Core`, `Roslyn`, `Patching`, `Agent`, and `Cli` now exist because their implemented authority boundaries require them. CLI's current production edges to Agent and Patching exist only to compose the internal one-target seam; they do not broaden Agent's Core-only dependency or grant it mutation authority. Future projects are added only when their implementing milestone demonstrates a real dependency or authority boundary.
