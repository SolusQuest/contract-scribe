...
A Scribe run may use bounded read-only tools to follow repository-defined context routes before it submits a documentation proposal. Multiple Scribe runs share immutable repository and scope context snapshots. They do not share hidden reasoning, mutable conversation history, or provider-owned session state. A single attempt may transiently replay its own bounded opaque assistant continuation when the selected thinking-mode tool protocol requires it; that state never crosses an attempt, run, target, or provider/model configuration boundary.
...
The M3 selection freezes the direct HTTP transport, exact DeepSeek/MiMo endpoint-model profiles, and shared provider-visible continuation shape. Future configuration or calibration changes must preserve the accepted bootstrap boundary below and require their own reviewed evidence.
...
An M3 provider-validation issue must define the executable request corpus, expected observations, normalization rules, failure taxonomy, and evidence needed before ContractScribe makes a compatibility or support claim. Provider-reported totals, usage, cache, and closed continuation statuses may be mapped into bounded ContractScribe-owned observations; raw assistant continuation and provider prose are not diagnostic or report fields.

The request construction, assistant message, tool-call, tool-result, retry, and terminal-response path remains shared unless executable validation demonstrates a real protocol difference. Configuration selects the documented thinking, reasoning-effort, tool-choice, continuation, and output-token fields without provider-name branching. One assistant response round owns one exact `content`/`reasoning_content` continuation envelope even when it contains parallel calls; later requests in that attempt replay it before the ordered tool results. It shares the normalized-response and logical-request bounds, is cleared on attempt restart, and is excluded from product messages, context identities, tools, evidence, proposals, reports, diagnostics, logs, and durable resume state. Usage response shapes are candidate normalization inputs rather than proof that usage accounting is the only divergence. ContractScribe does not create speculative provider branches, but it also does not describe unexecuted behavior as equal. Every observed difference must be recorded as compatibility evidence and reviewed before it changes the product contract.
