...
PR authors preserve the closed REST actor kinds User, Bot, Organization and Mannequin. GitHub's [migration mannequins](https://docs.github.com/en/migrations/overview/mannequins-and-user-activity) are current placeholder identities, not malformed actors: their exact bounded ID, node ID and login survive detail, pagination and duplicate comparison for R5 classification. The REST list schema additionally permits an explicit null author; enumeration preserves that absence without skipping the PR, inventing a ghost identity or inferring why the author is unavailable. Missing fields, malformed present actors and unknown kinds still fail closed. Identical absent authors deduplicate, while absent/present drift on a duplicate PR fails the complete set. Detail/create responses still require an actor under their distinct response schema. Mannequins and absent list authors are neither authenticated principals nor repository owners; the shared actor parser does not grant either authority. A mutation response containing an actor observation still requires owner readback and is never ownership proof.
...
The release workflow adds one bounded channel separate from the wrapper's acquisition channel: `prepare`'s `candidate` job builds credential-free, while its dedicated `attest` job holds only OIDC + attestation-write authority (no secrets, environment, or publication token) to emit and verify the payload's Sigstore provenance, and `stage-draft`/`promote` read the candidate artifact and perform release/tag mutations under the dedicated `RELEASE_PUBLICATION_TOKEN` — never the product token — exposed only to the mutation step's environment. Actions artifacts are transport, never authorization: release authority is the recomputed candidate digest plus the checked-in `payload-map.json` pair read at the published tag's own source revision, gated by the maintainer-provisioned environments and approval bound to that digest. No automation deletes, moves, or replaces release objects; ambiguous mutation outcomes stop for a fresh separately authorized dispatch.
