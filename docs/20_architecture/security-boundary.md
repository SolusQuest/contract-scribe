...
PR authors preserve the closed REST actor kinds User, Bot, Organization and Mannequin. GitHub's [migration mannequins](https://docs.github.com/en/migrations/overview/mannequins-and-user-activity) are current placeholder identities, not malformed actors: their exact bounded ID, node ID and login survive detail, pagination and duplicate comparison for R5 classification. Unknown kinds still fail closed. Mannequins are neither authenticated principals nor repository owners; the shared actor parser does not grant either authority. A mutation response containing an actor observation still requires owner readback and is never ownership proof.
