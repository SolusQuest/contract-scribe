...
`CreateTrustedProposal` first revalidates current C1 authority. It requires exact request/result/run-envelope correlation, the composition owner's current expected Tool Policy ID, the current work target, source, applicable components, and exact Style Profile. It projects only stable, source-free evidence metadata and the typed M2 block, including the bounded validated proposed structured content needed by M2. Prompt text, evidence or existing-documentation text, provider payloads, raw responses, diffs, source bytes, credentials, and process-local repository handles are excluded.
