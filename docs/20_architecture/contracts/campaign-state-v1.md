...
`CreateTrustedProposal` first revalidates current C1 authority. It requires an active provider reservation whose work-item key, exact request SHA, and M3 attempt identity match the proposal being admitted, plus exact request/result/run-envelope correlation, exact request limits, the composition owner's typed provider, model, Scribe protocol, and Tool Policy authority, the current work target, source, applicable components, and exact Style Profile. Those execution identities and the complete Scribe-limit projection enter the proposal commitment. An absent provider reservation, a patch reservation, or any substituted work, request, attempt, execution identity, or limit fails closed. It projects only stable, source-free evidence metadata and the typed M2 block, including the bounded validated proposed structured content needed by M2. Prompt text, evidence or existing-documentation text, provider payloads, raw responses, diffs, source bytes, credentials, and process-local repository handles are excluded.
...
For every patch attempt, `ReconstructPatchRequest` selects exactly work in `proposal-complete` or `accepted` status, preserves C1 order, and requires the fresh process to supply every and only the current typed evidence row for the persisted projection set. Each row must use the new `RepositoryContextRef` and exactly match the persisted subject, kind, relation, authority, stable locator, content commitment, byte/truncation observations, and claim categories. It then constructs the sorted distinct provenance catalog, writes the complete M2 request, and sends those exact bytes through `DocumentationPatchValidator.ParseRequest`. `ReconstructAcceptedPatchRequest` independently selects only `accepted` work and correlates the known complete candidate; only an accepted cumulative outcome is required to identify that accepted-only request, so a later rejected, stale, host-failed, cancelled, or timed-out mixed request cannot invalidate an earlier candidate. This distinction permits a known accepted candidate to coexist with newly proposal-complete work without pretending the unresolved next request already completed. Both projections prove the whole request against M2's 1 MiB, 512-block, 4,096-provenance, 64-reference-per-block, component, content, ordering, uniqueness, path, span, and vocabulary rules.

`CreatePatchReservation` derives the reservation request digest and expected revision from the exact active reconstruction and checkpoint; callers cannot supply those correlation facts. `CreatePatchCompletion` accepts only a result that passes `DocumentationPatchValidator.ValidateResult` for the exact request, derives the cumulative outcome and commitment, and, for an accepted result, derives accepted membership and every changed-file observation from that same result. The direct constructors for reservation, candidate, and cumulative-result DTOs are not public producer APIs. The domain-separated result commitment covers request identity, outcome, ordered target traces, changed-file facts, changed-block count, invariants, and bounded diagnostics.
