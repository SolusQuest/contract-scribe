...
4. `CampaignPlanningOwnerAuthoritySet`: every and only supported target, partitioned into snapshot-local physical-owner rows. Each target carries its exact classification, authoritative declaration ID, source kind and commitment, repository path and encoding or generated identity, M1 observation/full-declaration span, M2 requested/canonical/owner spans, attached documentation span, block state, writability, ordered applicable components and names, the exact owner `SymbolRef` set, multi-declarator and primary-constructor facts, and an exact target-valid `DocumentationScribeStyleProfile` when the target could be executable.
...
Ordering policy `campaign.order.complete-owner.ordinal.v1` uses a typed field comparer over complete physical-owner descriptors: explicit source-kind rank; locator/generated identity strings compared directly with `StringComparer.Ordinal`; exact source commitment and repository encoding; spans compared numerically; authoritative declaration ID; target `SymbolRef` and classification; ordered owner symbols and parent-scoped components; target Audit facts; violation causes; and terminal reasons. Length framing is used only in hash preimages and never as a lexicographic sort key. No array position, caller owner-equivalence label, culture, display name, clock, random state, machine path, or OS filesystem comparer participates.
...
The normative implementation is `CampaignPlanner` and its supporting types under `src/ContractScribe.Core/Campaign/Planning/**`. Conformance tests cover production-realistic M1 FullSpan versus M2 Span and leading-documentation topology, reason-aware Policy/Audit precedence, exact partial evidence bundles, relation-only mutations and relation input permutations, direct ordinal length-inversion paths, complete-block collapse, split/merged/extra owner attacks, exact bound-evidence and declaration correlation, substantive malformed targets, zero-component and multi-owner cases, known-answer commitments and keys, opaque-snapshot collision, exact source byte/encoding commitments, bounded canonical configuration JSON, duplicate-property privacy, finite aggregate bounds, opaque-ID privacy, Style Profile content binding and target association, valid and invalid input permutation, independent-process culture replay, empty work, stable fail-closed codes, privacy markers, and infrastructure dependency exclusion. Synthetic known-answer and adversarial case catalog facts live in `tests/fixtures/campaign/planning/vectors.json`; that fixture is test evidence, not a runtime schema.
