...
4. `CampaignPlanningOwnerAuthoritySet`: every and only target in the physical-owner closure of current Audit violation parents, partitioned into snapshot-local physical-owner rows. A nonviolating target appears only when it shares the selected physical owner and is named by that owner's exact `SymbolRef` closure; a skipped source-unavailable subject with no declaration carries no invented owner row. Each repository target carries a host-produced physical-source commitment plus its distinct lexical repository path, decoded observation-text commitment, exact file-byte commitment and encoding, exact classification, and authoritative declaration ID; generated targets carry their generated identity. Every selected target also carries the M1 observation/full-declaration span, M2 requested/canonical/owner spans, attached documentation span, block state, writability, ordered applicable components and names, the exact owner `SymbolRef` set, multi-declarator and primary-constructor facts, and an exact target-valid `DocumentationScribeStyleProfile` when the target could be executable.
...
The global authority graph must provide every and only current classification, relation, observation, bound-evidence row, and canonical Audit row once. The selected physical graph separately provides every and only violation parent plus its exact physical-owner `SymbolRef` closure. Target profile, classification fields including traits and skip facts, relation kind/source/target and canonical order, observation subjects and values, bound evidence authority/completeness/rows/IDs and complete serialized bundle, Audit row subjects/outcomes/reasons/unresolved locators, source identity, authoritative declaration ID, component parent/kind/identity/name, Style Profile component sequence, and selected owner membership must correlate exactly. Every `CampaignPlanningEvidenceAuthority` carries a C1-computed bounded projection over the complete current observation subject, value, completeness, unavailable cause, ordered declarations, source identities, text commitments, spans, block states, and component facts: commitment, declaration count, and conservative projection bytes are captured together, checked in aggregate before recomputation, recomputed from the accepted observation, and included in the execution commitment. Complete evidence must form a bijection among current declarations, declaration bindings, evidence-authority rows, selected evidence IDs, and bundle items; every item must match its declaration's required evidence kind and relation, exact text/SHA, source identity, locator/span, and UTF-8 counts. Non-complete evidence is validated item by item against the same current authority with rune-safe prefix and truncation accounting. This validation runs before policy-conflict and policy-unavailable precedence even when the Audit row intentionally omits selected evidence. Component observations must correlate to the same exact declaration and source authority selected for their parent target. Missing, extra, unselected, duplicate, stale, cross-session, substituted, contradictory, default, or unsupported data invalidates the plan.

The authority-owning host computes `physicalSourceCommitmentSha256` as lowercase SHA-256 over the domain-separated strict-UTF-8 preimage `contract-scribe/campaign/physical-source/v1\0` followed by the exact current load session's `PhysicalSourceIdentity`; raw machine paths do not enter Core. Before owner partitioning, Core builds one source-session index from every observation declaration. Each `CompilationContextRef` maps to exactly one observed project identity. Repository authority keeps `observedSourceTextSha256` (M1 SHA-256 of strict UTF-8 encoding of decoded syntax text) separate from `exactFileSha256` (M2/M3 SHA-256 of original repository bytes). Each canonical path maps to one observed-text SHA and one supplied physical-source/exact-file-SHA/encoding/writability tuple. Multiple lexical paths may map to the same physical-source commitment only when they agree on both decoded text and exact byte facts; the inverse physical commitment is unique over that complete tuple. UTF-8, UTF-8 BOM, UTF-16LE BOM, and UTF-16BE BOM therefore remain representable without equating distinct hash domains. Repository physical-owner identity is derived from the physical-source commitment and owner span. Generated sources are indexed and owned by a length-framed commitment over project identity, compilation-context ref, source kind, producer ID, and output ID, with exactly one generated-content commitment for that scoped identity. Every selected M2 source must correlate to this index; globally valid skipped observations need no fabricated physical source. Callers cannot supply an owner-equivalence identifier. Every owner row must contain a violation parent, exactly one derived physical descriptor, and the descriptor's complete sorted `SymbolRef` closure; no descriptor may appear in multiple rows and unrelated descriptors cannot share a row. Core derives the exposed `campaign-owner.<sha256>` from the validated physical descriptor and sorts rows by that descriptor before owner-scoped validation or identity construction.
...
Finite aggregate ceilings are 4,096 owners, 16,384 targets, 65,536 components, 65,536 relations, 65,536 unresolved classifications, 147,456 Audit rows, 81,920 bound-evidence rows, 65,536 owner-symbol memberships, and 131,072 Audit violations. Observation authority is limited to 4,096 declarations and 4 MiB conservative committed projection per observation, plus 65,536 declaration memberships, 65,536 complete evidence-authority declaration rows, and 32 MiB conservative committed projection in aggregate. These checks include project identity, compilation context, lexical/generated source identity, declaration IDs, commitments, and every other string present in the observation commitment; per-record limits are applied before sorting or hashing. Before canonical Audit materialization, the planner applies a conservative 32 MiB escaped-canonical byte ceiling independently to all supplied bound-evidence bundles and to the Audit authority; Audit traversal also enforces depth 128 and 2,000,000 structural tokens, and the produced canonical bytes are checked against the same 32 MiB ceiling. Configuration projections are limited to depth 32, 4,096 properties, 16,384 array items, 32,768 scalars, 64 KiB per scalar, 128 number characters, and 1 MiB conservative canonical UTF-8. Campaign patch bytes are at most 1 TiB; campaign input/output tokens and cost are at most `10^12`, `10^12`, and `10^15` microunits respectively; campaign elapsed time is at most 31 days. Scribe input/output tokens are at most `10^9`, elapsed milliseconds at most `2,000,000,000`, provider requests at most `10^6`, and attempts/candidates at most 1,000. Uncached tokens cannot exceed total input tokens. Global target-relevant component membership is indexed once by parent `SymbolRef`; owner validation performs one closure lookup per target and never rescans the global component array. Authority-graph validation is therefore linear apart from canonical per-collection sorting and evidence reconstruction. These are validation bounds, not permission to consume the maximum.

Validation category precedence is fixed and independent of caller collection order: root/snapshot/configuration and collection bounds; canonical Relation validation; aggregate observation/evidence count and byte proof; aggregate Audit depth/token/byte proof; canonical Audit materialization; bounded Classification/Observation exact-set projection; source-session graph construction; bound-evidence exact-set and complete/partial rebinding validation; reason-aware Audit correlation; then owner partition, source/declaration correlation, component, and Style Profile validation. Within owner validation, rows and targets are first ordered by canonical symbol keys, all source-session bindings close before physical-owner partitioning, and source checks precede component checks, which precede Style Profile checks. Multiple defects therefore cannot change the surfaced validation category merely by permuting input rows.

For current M1 semantics, target presence is authoritative when any declaration has `ParentSubstantive`, including a malformed block. Component presence requires a well-formed `ComponentMatch.Present`; without that positive fact, complete malformed component authority is unavailable. Ordinary Audit matrix rows correlate the exact observation, selected evidence authority, IDs, and bundle. `PolicyConflict` and `PolicyUnavailable` rows retain their contract-required null Audit observation and intentionally omitted selected evidence, but every included complete or partial evidence item is still rebound to current M1 declaration authority before that precedence is considered. `EvidenceIncomplete`, source-unavailable, and malformed rows require their forced unavailable Audit observation plus the exact reason-appropriate serialized evidence bundle. A supported source-unavailable skipped subject with no declarations remains identity-bound through the observation projection and canonical Audit digest but does not require physical-owner authority. Valid unsupported or precedence-skipped work does not suppress unrelated violations; a purported violation without owner authority fails closed.
...
Ordering policy `campaign.order.complete-owner.ordinal.v1` uses a typed field comparer over complete physical-owner descriptors: explicit source-kind rank; lexical locator/generated identity strings compared directly with `StringComparer.Ordinal`; repository physical-source commitment, exact source commitment and repository encoding; spans compared numerically; authoritative declaration ID; target `SymbolRef` and classification; ordered owner symbols and parent-scoped components; target Audit facts; violation causes; and terminal reasons. Generated physical identity is a length-framed commitment over its project/context-scoped source-session fields; this commitment is an identity field, not a substitute lexical sort key. Length framing is otherwise used only in hash preimages and never as a lexicographic sort key. No array position, caller owner-equivalence label, culture, display name, clock, random state, machine path, or OS filesystem comparer participates.
...
The normative implementation is `CampaignPlanner` and its supporting types under `src/ContractScribe.Core/Campaign/Planning/**`. Conformance tests cover production-realistic M1 FullSpan versus M2 Span and leading-documentation topology, reason-aware Policy/Audit precedence, same-subject cross-session complete and partial binding substitution, complete-bundle unselected-item rejection before policy precedence, changed declaration content and block authority, exact partial evidence bundles, relation-only mutations and permutations, direct ordinal length inversions, direct-path plus physical-source alias collapse, inverse path-to-physical conflict, conflicting context/project authority, project/context-scoped generated identity and conflicting generated content, split aliases and conflicting physical-source facts, observation projection per-record and aggregate bounds, complete-block collapse, split/merged/extra owner attacks, source-unavailable skipped targets beside unrelated violations, exact bound-evidence and declaration correlation, single-pass component-closure indexing, substantive malformed targets, zero-component and multi-owner cases, known-answer commitments and keys, opaque-snapshot collision, production-shaped UTF-8/UTF-8-BOM/UTF-16LE-BOM/UTF-16BE-BOM exact byte commitments distinct from observed-text hashes, bounded canonical configuration JSON, duplicate-property privacy, finite aggregate bounds, opaque-ID privacy, Style Profile content binding and target association, valid and invalid input permutation, independent-process culture replay, empty work, stable fail-closed codes, privacy markers, and infrastructure dependency exclusion. Synthetic known-answer and adversarial case catalog facts live in `tests/fixtures/campaign/planning/vectors.json`; that fixture is test evidence, not a runtime schema.
