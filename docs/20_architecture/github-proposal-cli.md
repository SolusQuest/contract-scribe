...
For initial/successor resume, accepted work is reconstructed before advancing remaining work. For append, an explicit preceding candidate equal to the current accepted origin selects further campaign progress. Once genuinely new work is accepted, its new origin selects reconstruction on an exact append retry. No remaining work produces a no-op, not another append of the same candidate. Accepted-only retry reuses the existing retry reservation/settlement owner and validates the accepted projection under the fresh process context.
