...
public sealed class GitHubPublicationProtocolDecisionHarnessTests
{
    ...
    [
        ...
        new("target-move-between-final-read-and-claim",
            Fault.TargetMoveBetweenFinalReadAndClaim),
        new("target-move-between-step-read-and-content-write",
            Fault.TargetMoveBetweenStepReadAndContentWrite),
        new("target-move-between-step-read-and-proposal-write",
            Fault.TargetMoveBetweenStepReadAndProposalWrite),
    ...
    public void Execute_three_alternatives_at_equivalent_serialized_race_windows()
    {
        ...
        Assert.All(results.Where(result => result.Scenario ==
                "target-move-between-final-read-and-claim"),
            result => Assert.Equal("one-admission-zero-resource-mutation", result.Residual));
        Assert.All(results.Where(result => result.Scenario ==
                "target-move-between-step-read-and-content-write"),
            result => Assert.Equal(
                "one-exact-orphan-content-zero-later-mutation", result.Residual));
        Assert.All(results.Where(result => result.Scenario ==
                "target-move-between-step-read-and-proposal-write"),
            result => Assert.Equal(
                "one-exact-stale-proposal-ref-zero-pr-mutation", result.Residual));

        var root = FindRepositoryRoot();
        var output = Path.Join(root, "TestResults", "issue-158-protocol-decision-v4",
            "transcripts.json");
    ...
    private static Observation Execute(IAlternative alternative, Scenario scenario)
    {
        ...
        var residual = "none";
        if (firstAdmitted && scenario.Fault == Fault.TargetMoveBetweenFinalReadAndClaim)
        {
            var gate = server.Send(alternative.RepositoryBoundAdmission
                ? WireRequest.ReadClaimAndBase(first)
                : WireRequest.ReadRepository(first));
            residual = gate.Outcome == "mismatch"
                ? "one-admission-zero-resource-mutation" : "unexpected-continuation";
            server.Record("continuation", "stale-operation",
                "fresh-generation-only");
        }
        else if (firstAdmitted
                 && scenario.Fault == Fault.TargetMoveBetweenStepReadAndContentWrite)
        {
            var gate = alternative.RepositoryBoundAdmission
                ? WireRequest.ReadClaimAndBase(first)
                : WireRequest.ReadRepository(first);
            Assert.Equal("exact", server.Send(gate).Outcome);
            Assert.Equal("admitted", server.Send(WireRequest.CreateContent(first,
                loseResponse: false)).Outcome);
            Assert.Equal("exact-recovered", server.Send(WireRequest.ReadContent(first)).Outcome);
            Assert.Equal("mismatch", server.Send(gate).Outcome);
            residual = "one-exact-orphan-content-zero-later-mutation";
            server.Record("continuation", "stale-operation",
                "fresh-generation-only");
        }
        else if (firstAdmitted
                 && scenario.Fault == Fault.TargetMoveBetweenStepReadAndProposalWrite)
        {
            var gate = alternative.RepositoryBoundAdmission
                ? WireRequest.ReadClaimAndBase(first)
                : WireRequest.ReadRepository(first);
            Assert.Equal("exact", server.Send(gate).Outcome);
            Assert.Equal("admitted", server.Send(WireRequest.UpdateProposal(first)).Outcome);
            Assert.Equal("exact-recovered", server.Send(WireRequest.ReadProposal(first)).Outcome);
            Assert.Equal("mismatch", server.Send(gate).Outcome);
            residual = "one-exact-stale-proposal-ref-zero-pr-mutation";
            server.Record("continuation", "stale-operation",
                "fresh-generation-only");
        }
        else if (firstAdmitted && scenario.Fault == Fault.ProposalRewindAfterFinalRead)
        {
    ...
    private static bool Evaluate(Fault fault, Metrics metrics) => fault switch
    {
        Fault.TargetMoveBetweenFinalReadAndClaim =>
            ...
            && metrics.Residual == "one-admission-zero-resource-mutation",
        ...
        Fault.TargetMoveBetweenStepReadAndContentWrite =>
            metrics.AdmittedOperations == 1 && metrics.ResourceWrites == 1
            && metrics.Residual == "one-exact-orphan-content-zero-later-mutation",
        Fault.TargetMoveBetweenStepReadAndProposalWrite =>
            metrics.AdmittedOperations == 1 && metrics.ResourceWrites == 1
            && metrics.Residual == "one-exact-stale-proposal-ref-zero-pr-mutation",
    ...
    private sealed class SerializedFaultServer
    {
        ...
        private bool resourceFaultInjected;
        ...
        private void InjectAdmissionFault(WireRequest request)
        {
            ...
            else if (AdmissionFault == Fault.TargetMoveBetweenFinalReadAndClaim)
            {
                TargetOid = Oid('9');
                RecordExternal("target-between-final-read-and-claim", TargetOid);
                admissionFaultInjected = true;
            }
        }

        private void InjectResourceFault(WireRequest request)
        {
            if (resourceFaultInjected) return;
            var contentWindow = AdmissionFault == Fault.TargetMoveBetweenStepReadAndContentWrite
                && request.Kind == "create-content";
            var proposalWindow = AdmissionFault == Fault.TargetMoveBetweenStepReadAndProposalWrite
                && request.Kind == "graphql-update-ref" && request.Resource == "proposal";
            if (!contentWindow && !proposalWindow) return;

            TargetOid = Oid('9');
            RecordExternal(contentWindow
                ? "target-between-step-read-and-content-write"
                : "target-between-step-read-and-proposal-write", TargetOid);
            resourceFaultInjected = true;
        }
    ...
    private enum Fault
    {
        ...
        TargetMoveBetweenFinalReadAndClaim,
        TargetMoveBetweenStepReadAndContentWrite,
        TargetMoveBetweenStepReadAndProposalWrite,
